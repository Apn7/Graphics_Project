...
struct LightState {
    ...
    glm::vec3 SpotLightColor   = glm::vec3(5.0f, 4.5f, 3.2f);  // Very bright warm hotspot — clearly visible spotlight
    float     SpotCutoff       = 0.9397f;   // cos(20°) — wider bright core
    float     SpotOuterCutoff  = 0.8192f;   // cos(35°) — wide soft penumbra
    float     SpotConstant     = 1.0f;
    float     SpotLinear       = 0.04f;     // Low falloff — light reaches the whole desk surface
    float     SpotQuadratic    = 0.01f;
